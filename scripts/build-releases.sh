#!/bin/sh
set -eu

ROOT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
DIST_DIR="$ROOT_DIR/dist"
STAGE_DIR="$ROOT_DIR/.build/package"
TEMPLATE_ZIP="$DIST_DIR/CoreShift_Update_Patcher_Template_v1.0.zip"
INOI_ZIP="$DIST_DIR/CoreShift_Update_Patcher_INOI_A75_Elegance_Runtime_Fixes_v1.0.zip"

cd "$ROOT_DIR"

rm -rf "$STAGE_DIR"
mkdir -p "$STAGE_DIR" "$DIST_DIR"

copy_path() {
  src=$1
  [ -e "$src" ] || return 0
  dst="$STAGE_DIR/$src"
  mkdir -p "$(dirname "$dst")"
  cp -a "$src" "$dst"
}

copy_path module.prop
copy_path manifest.json
copy_path META-INF
copy_path payload

rm -f "$TEMPLATE_ZIP" "$INOI_ZIP"

(
  cd "$STAGE_DIR"
  find . -type f | sort | sed 's#^\./##' | zip -q -X "$TEMPLATE_ZIP" -@
)

cp "$TEMPLATE_ZIP" "$INOI_ZIP"

echo "SHA256:"
sha256sum "$TEMPLATE_ZIP" "$INOI_ZIP"

if cmp -s "$TEMPLATE_ZIP" "$INOI_ZIP"; then
  echo "INOI release ZIP matches template ZIP byte-for-byte."
else
  echo "INOI release ZIP differs from template ZIP." >&2
  exit 1
fi
